...
export default function AdvertisingPage() {
  ...
  useEffect(() => {
    const fetchAds = async () => {
      try {
        // ⬇️ keep same pattern as films/episodics
  ...
  if (loading) {
    return <p className="text-white text-center mt-10">Loading advertising...</p>;
  }
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {ads.map((ad, idx) => (
            ...
            >
              {/* Poster with hover trailer (same interaction) */}
              <div className="relative w-full h-[350px] overflow-hidden rounded-md group">
                ...
                <iframe
                  ...
                  src={`${ad.trailerUrl}?autoplay=1&mute=1`}
                  title={`${ad.title} Trailer`}
                  allow="autoplay; encrypted-media"
                ></iframe>
              </div>

              <div className="mt-4">
                <h2 className="text-lg font-bold text-yellow-400">{ad.title}</h2>
                <p className="text-gray-300 text-sm mt-1">{ad.description}</p>
                {/* keep rating optional if your API provides it */}
                {ad.rating != null && (
                  <p className="text-blue-400 font-semibold mt-2">⭐ {ad.rating}/10</p>
                )}
              </div>
            </motion.div>
          ))}
